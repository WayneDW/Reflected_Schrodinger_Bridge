...
for i in range(2):
    ...
    t0 = np.random.choice([1e-4, 3e-5])
    sde_type = np.random.choice(['vp'])
    num_itr = np.random.choice([800])
    stage = np.random.choice([30])
    ...
    if sde_type == 've':
        ...
        beta_min = np.random.choice([0.04, 0.05])
        beta_max = np.random.choice([0.5])
